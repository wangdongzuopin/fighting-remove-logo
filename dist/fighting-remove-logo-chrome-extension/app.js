...
function resetWorkspace() {
  ...
  uploadSurface.classList.remove("has-image");
  state.file = null;
...
function activateStage() {
  ...
  uploadSurface.classList.add("has-image");
}
...
uploadSurface.addEventListener("click", (event) => {
  if (!state.file) {
    openFilePicker();
    return;
  }

  if (event.target === uploadSurface || emptyState.contains(event.target)) {
    openFilePicker();
  }
});
